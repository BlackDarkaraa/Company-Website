...
jQuery.fn.isOnScreen = function(){
    ...
    if (this.length) {
        ...
        if((!(viewport.right < bounds.left || viewport.left > bounds.right || viewport.bottom < bounds.top || viewport.top > bounds.bottom)) === true) {
            return this;
        } else {
            return false;
        }
...
jQuery(document).ready(function($) {
    ...
    $('.menu>li').on('mouseenter touch', { passive: true } , function(e) {

        $('.menu').children('li').removeClass('active').addClass('inactive');
        $('.main-menu__mega').removeClass('active');

        if ($(this).hasClass('mega-selector')) {
            var $menu_id = $(this).data('mega-menu');
            $(this).removeClass('inactive');


            var $offset = $(this).offset();
            var $menu_offset = $('.main-menu__mega[data-mega-id=' + $menu_id +']').offset();

            //var $offset = String($offset.left - $menu_offset.left) + 'px';

            //console.log($offset);

            //console.log($menu_offset);

            //$('.main-menu__mega[data-mega-id=' + $menu_id +']').addClass('active').css('left', $offset);
            $('.main-menu__mega[data-mega-id=' + $menu_id +']').addClass('active').offset({ left: $offset.left } );
        ...
        if (!$('body').hasClass('off-canvas-open')){
            ...
            //$(this).children('.submenu-arrow').trigger('click');
            //$(this).removeClass('inactive').addClass("active");
        }
    }); 

    $('.menu-watch').on('mouseenter touch', { passive: true } , function(e) {
        $('.progress-container').removeClass('active');
    });           

    //$('.main-menu__mega').not('.menu').on('mouseleave touch', { passive: true } , function(e) {
    $('.menu-watch').on('mouseleave touch', { passive: true } , function(e) {

        var element = e.toElement || e.relatedTarget;

        if(!$(element).hasClass('menu-watch')){
            $('.main-menu__mega').removeClass('active');		      
            $('.menu').children('li').removeClass('active inactive');
...
/*    $('#site-navigation').not('.main-menu__mega').on('mouseleave touch', { passive: true } , function(e) {
        $('#header').removeClass('menu-active');
        $('.main-menu__mega').removeClass('active');
        $('.menu').children('li').removeClass('active inactive');
    });*/
    ...
    $('.main-menu__mega-links a, .main-menu__mega-links .a').on('mouseenter touch', { passive: true } , function(e) {
        var $submenu_id = $(this).closest('li').data('sublink');
        $(this).closest('.main-menu__mega-links').find('.a').removeClass('arrow-active active');
        $(this).closest('.main-menu__mega-section').next().find('.main-menu__mega-sublinks').removeClass('active');	

		$(this).addClass("active");
        $('.main-menu__mega-sublinks[data-sublink-id=' + $submenu_id +']').addClass('active');
    });  


    // set mega sections all to the same height

    resize_menu();

    //$('.main-menu__mega').addClass('processed');

    //$('.main-menu__mega-section').addClass('processed');
    ...
    $(window).on('resize', function() {
        ...
        menu_resize_timer = setTimeout(function() { 
            if ( ($(window).width() > 1023) && (menu_mobile_resize == true)) {
                menu_mobile_resize = false;
                resize_menu();
            }
            if ( ($(window).width() < 1024) && (menu_mobile_resize == false) ) {	
    ...
    function resize_menu() {
        ...
        var selector = $('.main-menu__mega-section');
        selector.each(function(){
    
            
            //var width = $(this).width();
            var sublinks = $(this).find('.main-menu__mega-sublinks');
            var heights = [];   
            //var widths = [];  
...
//        $(this).toggleClass('active');
        $('.accordion dt').not($(this)).removeClass('open active');
        //$('.accordion dd').not($(this)).removeClass('open');
...
jQuery(document).ready(function($) {
    ...
    $('body').on('click', '.video a, #show-video', function(e) {
        e.preventDefault();
        show_lightbox($(this));
    });
    
    function show_lightbox($object) {
        var id = $($object).data('video');
        var id2 = $($object).data('video-2');
	...
	$('body').on('click', 'a.gmap-pu', function(){
	...
	$('body').on('submit', '.contact-form', function(){
        ...
        $($current_form).find('div.form-error').html('').hide()
		...
		$.ajax({
			...
			type : 'POST',
			beforeSend : function ( xhr ) {
				$($current_form).find('.submit').prop('disabled', true).addClass('disabled');
			},
			success : function( response ){ 
                var data = response.data;
				//console.log (data);
				
				$current_form.find('.submit').prop('disabled', false).removeClass('disabled');

                if (data.error == 1) {
                    if (data.first_name) {
                        $($current_form).find('input[name="first_name"]').siblings('div.form-error').html(data.first_name).show();
                    }
                    if (data.last_name) {
                        $($current_form).find('input[name="last_name"]').siblings('div.form-error').html(data.last_name).show();
                    }
                    if (data.email_address) {
                        $($current_form).find('input[name="email_address"]').siblings('div.form-error').html(data.email_address).show();
                    }
                    if (data.phone_number) {
                        $($current_form).find('input[name="phone_number"]').siblings('div.form-error').html(data.phone_number).show();
                    }
                    if (data.company) {
                        $($current_form).find('input[name="company"]').siblings('div.form-error').html(data.company).show();
                    }
                    if (data.message) {
                        $($current_form).find('textarea[name="message"]').siblings('div.form-error').html(data.message).show();
                    }
                } else {
                    if (data.sent) {
                        ...
                        if ( (data.form_id != 'form-3' && data.form_id != 'form-6' && data.form_id != 'form-8' && data.form_id != 'form-5') && data.unique_id) {
                            //var form_data = data.form_data;
                            //console.log(form_data);
                            //console.log('F24 sent');
                            //f24("send", "form", "#" + data.unique_id );
                        }

                        var message_title = (data.response_title ? '<h2>' + data.response_title + '</h2>' : '');
                        //var message = data.response.replace(/\'/g, "'");
                        ...
                        if (data.dl_link) {
                            button = '<a href="' + data.dl_link + '" class="button" target="_blank">Download Now</a>';
                        } else if (data.video_link) {
                            video_link = '<a href="#" class="button" id="show-video" data-video="' + data.video_link + '" data-video-2="' + data.video_link_2 + '" target="_blank">View Video</a>';
                            button = '<div class="video">' + video_link +  '</div>';
                        }

                        var msg = '<div class="form-message success">' + message_title + '<p>' + message + '</p>' + (button ? button : '') + '</div>'

                        $($current_form).closest('.form-wrapper').html(msg);

                        if (video_link.length) {
                            show_lightbox($(video_link));
                        }

                    } else {
                        $($current_form).prepend('<div class="form-message fail"><p>' + data.response + '</p></div>');
					}
				}
    ...
    $('body').on('change', '#cats', function(){

        var id = $(this).children("option:selected").val();

        if ((!$(this).hasClass('category')) && (id != '-1')) {
            //console.log(id);
            ...
            $.ajax({
                ...
                success : function( response ){ 
                    ...
                    if (data.output) {
                        $(".post-wrapper .container").html(data.output);
                    } else {
...
jQuery(document).ready(function($) {
    ...
    if (Cookies.get('aws-cookie-ver') != '2') {
        Cookies.remove('aws-cookie-consent');
        $('.aws-cookie-consent').show();
    }

    if ( Cookies.get('aws-cookie-consent') != '1' || Cookies.get('aws-cookie-consent') != '2' ) {
        $('.aws-cookie-consent').show();
    }

    $('#aws-cookie-accept, #aws-cookie-accept-page').on('click touch',function(e) {
        Cookies.set('aws-cookie-consent', '1',  { expires: 365, sameSite: 'lax', secure: true });
        Cookies.set('aws-cookie-ver', '2',  { expires: 365, sameSite: 'lax', secure: true });
    ...
    $('#aws-cookie-agree, #aws-cookie-agree-page').on('click touch',function(e) {
        Cookies.set('aws-cookie-consent', '2',  { expires: 365, sameSite: 'lax', secure: true });
        Cookies.set('aws-cookie-ver', '2',  { expires: 365, sameSite: 'lax', secure: true });
    ...
    $('#aws-cookie-reset').on('click touch',function(e) {
        Cookies.remove('aws-cookie-consent');
...
jQuery(document).ready(function($) {
    ...
    $('.submenu-arrow').click(function(){
        ...
        $('li[data-level="'+$parent_level+'"] ul').not($submenu).removeClass("expanded");
        $('li[data-level="'+$parent_level+'"] ul').not($submenu).slideUp(); //hide the others			
...
/*    if ( Cookies.get('eipmodal') == 'true' ) {
        Cookies.set("eipmodal", -1);
    }

    cnt = parseInt(Cookies.get('eipmodal')) + 1;
    Cookies.set("eipmodal", cnt);    

    if ( (cnt % 2) == 0 ) {*/
        jQuery('.pu-overlay').addClass('active');
        jQuery("#bounceModal").addClass("active");
    //}
}  
...
jQuery(document).ready(function($) {
    ...
    addEvent(document, "mouseout", function(e) {
        ...
        //if ( (!from || from.nodeName == "HTML") && (e.pageY < $(window).scrollTop()) && ($.cookie("eipmodal") != 'true') ) {
        if ( (!from || from.nodeName == "HTML") && (e.pageY < $(window).scrollTop()) ) {
            //$.cookie("eipmodal", 'true', { path: '/' });
            console.log('Exit Intent');
            $("#eipModal").addClass("active");
        }
    });
...
jQuery(document).ready(function($) {
    ...
    $('#art-side-form').css('display','block');
    ...
    $('#art-side-form .close-side').on('click touch',function(e) {
        e.preventDefault();
        ...
        if ($el == '#art-side-form') {
            Cookies.set('side-bar-form', 'closed', { expires: 1 });
        }
    }); 
    ...
    $(window).scroll(function(){
        ...
        if (!$el.data('closed') == '1') {
            ...
            if ($status != 'closed') {
                if ($el.length && $(window).width() > 1023) {
                    if (( $('.hero-slider').isOnScreen() || $('#get-in-touch').isOnScreen() || $('.ecmff-wrapper').isOnScreen() )) {
                        if (!$el.hasClass('opened')) {
                            $el.removeClass("open");
                        }
                        if ($('#get-in-touch').isOnScreen()) {
                        ...
                        if ($('.scroll-down').length) {
                            var elementOffset = $('.scroll-down').offset().top;
                        }
        ...
        if ($(window).width() < 1024) {
            if (( $('.hero-slider').isOnScreen() || $('#get-in-touch').isOnScreen())) {

                if (!$('#art-side-form').hasClass('open')) {
                    $('#art-side-form-tab').hide();
                }
            } else {
                $('#art-side-form-tab').show();
            }
        }
    ...
    var side_form_resize_timer, side_form_mobile_resize = false;

    $(window).on('resize', function() {
        clearTimeout(side_form_resize_timer);                 
        side_form_resize_timer = setTimeout(function() { 
            if ( ($(window).width() < 1024) && (side_form_mobile_resize != true)) {
                if ($('.hero-slider').isOnScreen()) {
                    $('#art-side-form-tab').hide();
                }
            }
            if ( ($(window).width() > 1023) && (side_form_mobile_resize == true) ) {	
                $('#art-side-form-tab').show();
            }                     
        }, 200);	
    }).resize();	
    ...
    var timed = $('#art-side-form').data('timed');

    $closed_status = Cookies.get('side-bar-form');

    if (timed && !($closed_status == 'closed') && ($(window).width() > 1023)) {
        setTimeout(function() { 
            $('#art-side-form').addClass('open');
        }, timed * 1000);
    }
...
jQuery(document).ready(function($) {
    ...
    var headerHeight = $("#header").height() + adminHeight;
...
//	  console.log('NO Obejct Fit');
	...
	$('#map-show-locations').on('click touch', { passive: true } , function(e) {
    ...
    $(".read-more-toggle").click(function(e) {
        ...
        //$(this).next(".read-more-wrapper").slideToggle();
        var anchor = $(this).prev(".readmore-anchor");

        if ($(this).hasClass("expanded")) {
            var readmoreDiv = $(this).next(".read-more-wrapper");
            $(this).find($('.text')).html(lesstext);
            var readmoreButton = $(this).detach();
            readmoreDiv.after(readmoreButton);            
            readmoreDiv.slideToggle();
        } else {
            var readmoreDiv = $(this).prev(".read-more-wrapper");
            $(this).find($('.text')).html(moretext);
            var readmoreButton = $(this).detach();
            readmoreDiv.slideToggle();
            setTimeout(function() { 
                readmoreDiv.before(readmoreButton);  
            }, 300);          
    ...
    $('.art-social-link').click(function(e){
        e.preventDefault;
        var w = 500;
        var h = 300;
        var left = Number((screen.width/2)-(w/2));
        var tops = Number((screen.height/2)-(h/2));        
        window.open(this.href,'popUpWindow','width='+w+', height='+h+', top='+tops+', left='+left+',resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=yes');
        return false;
    })    

    /* Hero full height mobile */

    function calcVH() {
        var minHeight;

        minHeight = $('.slider-type-2.normal .single-slide .slide-content-wrapper').innerHeight();
        minHeight += $('.slider-type-2.normal .single-slide .awards').innerHeight();
        minHeight += $('#header').innerHeight() + 90;
        $('.slider-type-2.normal .single-slide .slide-image').removeAttr('style');
        $('.slider-type-2.normal .single-slide .slide-image').css('min-height', minHeight +'px');

        var minHeight = $('.slider-type-3.normal .single-slide .slide-content-wrapper').innerHeight();
        minHeight += $('.slider-type-3.normal .single-slide .awards').innerHeight();
        minHeight += $('#header').innerHeight() + 90;
        $('.slider-type-3.normal .single-slide .slide-image').removeAttr('style');
        $('.slider-type-3.normal .single-slide .slide-image').css('min-height', minHeight +'px');

        $('.slider-type-3.normal .single-slide .slide-image').innerHeight( $(this).innerHeight() );
		$('.slider-type-2.normal .single-slide .slide-image').innerHeight( $(this).innerHeight() );
    }

    function calcVHCondense() {
        var minHeight;

        minHeight = $('.slider-type-2.normal .single-slide .slide-content-wrapper').innerHeight();
        minHeight += $('#header').innerHeight() - 32;
        $('.slider-type-2.normal .single-slide .slide-image').removeAttr('style');
        $('.slider-type-2.normal .single-slide .slide-image').css('height', minHeight +'px');

        var minHeight = $('.slider-type-3.normal .single-slide .slide-content-wrapper').innerHeight();
        minHeight += $('#header').innerHeight() - 32;
        $('.slider-type-3.normal .single-slide .slide-image').removeAttr('style');
        $('.slider-type-3.normal .single-slide .slide-image').css('height', minHeight +'px');
    }    

    var hero_resize_timer, hero_mobile_resize = false;

    $(window).on('resize', function() {
        clearTimeout(hero_resize_timer);                 
        hero_resize_timer = setTimeout(function() { 
            if ( ($(window).width() < 768) ) {
                if ($('.nwgju-wrapper.hero-condense-for-mobile').length == 0) {
                    calcVH();
                } else {
                    //calcVHCondense();
                }
                hero_mobile_resize = true;
            }
            if ( ($(window).width() > 767) && (hero_mobile_resize == true) ) {	
                hero_mobile_resize = false;
                $('.slider-type-2 .single-slide .slide-image').css('height', '');
                $('.slider-type-3 .single-slide .slide-image').css('height', '');
            }                     
        }, 200);	
    }).resize();	    
    ...
    /*/ rubaw section buttons */

    $('.selector.resource').on('click touch', { passive: true } , function(e) {
        ...
        if (!$(this).hasClass('active')) {
            ...
            $(this).addClass('active');
            //$('.resource-panel').hide(0);
            $('.resource-panel').fadeTo('fast', 0);
            //$('.resource-panel').animate({width:'toggle'},350);
            //$('.resource-panel').not($('#' + $(this).data('id'))).fadeTo('fast', 0); 
            $('#' + $(this).data('id')).fadeTo('fast', 100); 
            $('.resource-panel').not($('#' + $(this).data('id'))).hide(); 
            //$('#' + $(this).data('id')).animate({width:'toggle'},350);
            
                      
        }
       
        $(this).parent('.custom-select_options').slideUp('fast');
        $(this).parent('.custom-select_options').prev('.custom-select_selected').find('.selector-select').html($(this).html());
    });    


    /*/ wqlio section buttons (use cases) */

    $('.selector.use-case a').on('click touch', { passive: true } , function(e) {

        e.preventDefault();

        if (!$(this).closest('.selector').hasClass('active')) {
            $('.selector.use-case').removeClass('active');
            $(this).addClass('active');                      
        }
       
        $(this).closest('.custom-select_options').slideUp('fast');
        $(this).closest('.custom-select_options').prev('.custom-select_selected').find('.selector-select').html($(this).html());
    }); 

    // redirect to blog category
    $('#cats.category').on('change', function() {
    ...
    $('.custom-select_selected').on('click touch', { passive: true } , function(e) {
        $(this).next('.custom-select_options').slideToggle('fast');
    });


    /*/ daoet bullet panels */

    $('.daoet-wrapper .bullet-title').on('click touch', { passive: true } , function(e) {

        e.preventDefault();

        if (!$(this).hasClass('active') && !($(this).hasClass('mobile-active') && $(document).width() < 767 )) {
            $('.daoet-wrapper .bullet-title').removeClass('active mobile-active');
            $(this).addClass('active');
            $('.daoet-wrapper .bullet-panel').removeClass('active mobile-active').slideUp(200);
            var panel = $('#' + $(this).data('id'));
            panel.slideDown(200, function() {
                panel.addClass('active');
            });    
        } else {
            $(this).removeClass('active mobile-active');
            var panel = $('#' + $(this).data('id'));
            panel.slideUp(200, function() {
                $('.daoet-wrapper .bullet-panel').removeClass('active mobile-active');
            });    


            //$('#' + $(this).data('id')).slideUp(200, function() {
                //$(this).removeClass('active mobile-active');
            //});       
        }
    });

    $('.daoet-wrapper .bullet-panel .close').on('click touch', { passive: true } , function(e) {

        e.preventDefault();

        $('.daoet-wrapper .bullet-title').removeClass('active');
        $('.daoet-wrapper .bullet-panel').slideUp(200);
    });


    /*/ ktyeh bullet panels */

    $('.ktyeh-wrapper .bullet-title').on('click touch mouseover', { passive: true } , function(e) {
        ...
        if (!$(this).hasClass('active') ) {
            $('.ktyeh-wrapper .bullet-title').removeClass('active');
            $(this).addClass('active');
            $('.ktyeh-wrapper .bullet-panel').removeClass('active');
    ...
    $('.bcola-wrapper .image-link').on('click touch', { passive: true } , function(e) {

        e.preventDefault();

        if (!$(this).hasClass('active')) {
            $('.bcola-wrapper .image-link').removeClass('active');
            $(this).addClass('active');
            $('.bcola-wrapper .image-link .content').hide();
            $('.bcola-wrapper .image-link-wrapper .image').hide();
            $(this).find('.content').slideDown(200);
            $('#' + $(this).data('link-id')).fadeIn(300);     
            $('.bcola-wrapper .map .image-wrapper').addClass('opacity');
        } else {
            $(this).find('.content').slideUp(200, function() {
                $('.bcola-wrapper .image-link').removeClass('active');
            });
            $('#' + $(this).data('link-id')).fadeOut(300);    
            $('.bcola-wrapper .map .image-wrapper').removeClass('opacity');
        }
    });


    // masonry for services

    var leftColumnHeight = 0,
    rightColumnHeight = 0,
    $articles = $('.services-main-wrapper .column.service');
    ...
    $('.show-team-member').on('click touch', { passive: true } , function(e) {
        e.preventDefault();
        $('.pu-overlay').addClass('active');
    ...
    $( document ).on('click touch', '.pu-overlay, .pu-wrapper, .close-modal, .team-member-pu', { passive: true } , function(e) {
        close_pu();
    });


    $('.close-modal').on('click touch', { passive: true } , function(e) {
        close_pu();
    });

    function close_pu() {
        $('.team-member-pu').removeClass('active');
        $('.pu-overlay').removeClass('active');
        $('#pu-temp').remove();
        $('.pu-wrapper').not('.team-member-pu').remove();
    }
...
// flipper
    ...
    $( document ).on('click touch', '.flipper', { passive: true } , function(e) {
        $(this).toggleClass('hover');
        $(this).closest('.box').toggleClass('hover');
    });


    $( document ).on('click touch', '#top-bar .cross', { passive: true } , function(e) {
        //$('#top-bar').hide();
        $('#top-bar').remove();
        $('body').removeClass('with-top-bar');

        if ( Cookies.get('aws-cookie-consent') == '1' || Cookies.get('aws-cookie-consent') == '2' ) {
            Cookies.set('topbar-hidden', '1',  { sameSite: 'lax', secure: true });
        }
    ...
    window.onscroll = function() {
        ...
        if ( $(document).width() > 1023 ) {
            if ( topHeight && (scrolled > topHeight )) {
        ...
        if ($('.wqlio-wrapper').length) {
            $( ".card-single" ).each(function() {
                var scrollTop     = $(window).scrollTop();
                ...
                if( $( this ).isOnScreen() ) { 
                    ...
                    if ( $(document).width() < 767 ) {
                        if (distance < 150 && distance  > 0) {
                            ...
                            $('.custom-select_options').prev('.custom-select_selected').find('.selector-select').html($(this).data('title'));
                        }
                    } else {
                        if (distance < 150 && distance  > 0) {
                            $('.sticky-side-menu a').removeClass('active');
                            $('[data-id=' + $(this).attr('id') + ']').addClass('active');
                        }
                    }
                }
    ...
    $('textarea').on("input", function(){
        ...
        if (maxlength) {
            ...
            $(this).next('.charCnt').text(charsLeft + " characters left");
...
$(window).on( "load", function() {
    var topbar_active = $('body').hasClass('top-bar-active');

    if ((Cookies.get('topbar-hidden') != '1') && topbar_active) {
...
jQuery(document).ready(function($) {
    //console.log('removing fouc');
    $('html').removeClass('fouc_hidden');
    $('html').addClass('fouc_fadein');
});
jQuery(function($) {

    $('.play-video').on('click touch', { passive: true } , function(e) {
		e.preventDefault();
        ...
        var iframe = '<iframe style="width: 100%; height: ' + height + 'px;" src="https://www.youtube.com/embed/' + src + '?rel=0&showinfo=0&autoplay=1" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
...
jQuery(function($) {
    ...
    $(window).scroll(function(){
        var obj = $('.fade-page').isOnScreen();
        /*console.log( 'Obj offset '+obj.offset().top)
        console.log( 'Scrolltop '+$(window).scrollTop())
        console.log( 'Window Height '+$(window).height())
        console.log( 'Obj height ' +obj.height())
        console.log( 'Bottom ' + ( $(window).scrollTop() + $(window).height() - obj.offset().top ));*/
        if (obj && !scrolled) {
            var bottom = $(window).scrollTop() + $(window).height() - obj.offset().top;
            if (bottom > 200) {
                var fadeTime = (obj.data('fade-time') ? obj.data('fade-time') : 5) * 1000;
                scrolled = true;
                obj.css('z-index', '10001');
                $('.pu-overlay').addClass('active').delay(fadeTime).queue(function(n) {
                    $('.pu-overlay').removeClass('active');
